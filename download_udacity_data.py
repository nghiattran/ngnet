...
def handle_data(udacity_dec_dir, type):
    ...
    data_extected_folder = os.path.join(udacity_dec_dir, extracted_dirname)
    if os.path.exists(data_extected_folder):
        logging.info("Udacity %s extracted data FOUND at %s." % (type, data_extected_folder))
    else:
    ...
    if os.path.exists(converted_path):
        logging.info("Udacity %s converted data FOUND at %s." % (type, converted_path))
    else:
        try:
    ...
    with open(os.path.join(converted_path, 'train.txt'), 'r') as f:
        ...
        for id in data:
            id = id[:-1]
            entry = '%s %s' % (os.path.join(type.lower(), 'training', 'image_2', id + '.jpg'),
                               os.path.join(type.lower(), 'training', 'label_2', id + '.txt'))

...
def _build_inner_layer(hyp, encoded_features, train):
    ...
    with tf.variable_scope('Overfeat', initializer=initializer):
        ...
        b = tf.get_variable('ip_bias', shape=[hyp['num_inner_channel']])
        output = tf.matmul(hidden_input, w) + b

    if train:
        # Adding dropout during training
        output = tf.nn.dropout(output, hyp['solver'].get('keep_prob', 0.5))
    return output
...
def _build_output_layer(hyp, hidden_output):
    ...
    box_bias = tf.get_variable('box_out_bias',
                               shape=(4))

    conf_weights = tf.get_variable('confs_out',
                                   shape=(hyp['num_inner_channel'],
                                          hyp['num_classes']))
    conf_bias = tf.get_variable('confs_out_bias',
                                shape=(hyp['num_classes']))

    pred_boxes = tf.reshape(tf.matmul(hidden_output, box_weights) * 50 + box_bias,
                            [outer_size, 1, 4])

    # hyp['rnn_len']
    pred_logits = tf.reshape(tf.matmul(hidden_output, conf_weights) + conf_bias,
...
def _build_rezoom_layer(hyp, rezoom_input, train):
    with tf.name_scope('rezoom_layer'):
        ...
        rezoom_features = _rezoom(
            ...
            w_offsets, h_offsets)
        if train:
            rezoom_features = tf.nn.dropout(rezoom_features, hyp['solver'].get('keep_prob', 0.5))
        ...
        delta_weights1 = tf.get_variable('delta1',
                                         shape=shape)
        delta_bias1 = tf.get_variable('delta1_bias',
                                         shape=[shape[-1]])
        ...
        delta_confs_bias = tf.get_variable(
            'delta2_bias', shape=[hyp['num_classes']])

        delta_boxes_weights = tf.get_variable('delta_boxes', shape=[dim, 4])
        delta_boxes_bias = tf.get_variable('delta_boxes_bias', shape=[4])

        rere_feature = tf.matmul(ip1, delta_boxes_weights) * 5 + delta_boxes_bias
        pred_boxes_delta = (tf.reshape(rere_feature, [outer_size, 1, 4]))

        scale = hyp.get('rezoom_conf_scale', 50)
        feature2 = tf.matmul(ip1, delta_confs_weights) * scale + delta_confs_bias
...
def decoder(hyp, logits, train):
    ...
    initializer = tf.contrib.layers.xavier_initializer()
    l2_reg = tf.contrib.layers.l2_regularizer(hyp.get('reg_strength', 1e-6))
...
def evaluation(hyp, images, labels, decoded_logits, losses, global_step):
    ...
    eval_list.append(('En. Weight', losses['encoder_reg_loss']))
    eval_list.append(('De. Weight', losses['decoder_reg_loss']))
    if hyp['use_rezoom']:
